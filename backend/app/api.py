...
def health_check():
    ...
    This endpoint verifies that the API is running and returns its health
    status and current version.

    Returns:
        HealthResponse: An object containing the health status, which is
            always "healthy", and the current version of the API.
    """
...
def list_prompts(
    ...
    This endpoint fetches all available prompts and can filter them based on
    the provided parameters. It returns a list of prompts wrapped in a
    PromptList model.

    Args:
        collection_id (Optional[str]): An optional collection ID to filter
            prompts that belong to a specific collection.
...
def create_prompt(prompt_data: PromptCreate):
    ...
    This endpoint allows the creation of a new prompt. If a collection ID is
    provided, it checks for the existence of the collection.

    Args:
        prompt_data (PromptCreate): The data required to create a new prompt,
            including title, content, optional description, and collection ID.
...
def update_prompt(prompt_id: str, prompt_data: PromptUpdate):
    ...
    This endpoint updates the details of an existing prompt identified by its
    ID. If a collection ID is provided, the existence of the collection is
    checked.
    ...
    Args:
        ...
        prompt_data (PromptUpdate): The new data for the prompt, including
            title, content, description, and optional collection ID.
...
def partial_update_prompt(prompt_id: str, prompt_data: PromptUpdate):
    ...
    This endpoint allows for partial updates to a prompt's information,
    including title, content, description, and collection association.

    Args:
        prompt_id (str): The unique identifier of the prompt to update.
        prompt_data (PromptUpdate): An object containing the new values.
    ...
    update_data.update({
        k: v
        for k, v in prompt_data.model_dump().items()
        if v is not None
    })
...
@app.put(
    "/prompts/{prompt_id}/versions/{version_id}/revert",
...
async def revert_version(prompt_id: str, version_id: str) -> Prompt:
    ...
    if (
        not version_to_revert
        or version_to_revert.prompt_id != prompt_id
    ):
        raise HTTPException(
            status_code=404,
            detail="Version not found or does not match prompt."
        )
...
def get_collection(collection_id: str):
    ...
    This endpoint fetches a specific collection using the provided ID. If no
    collection is found, a 404 error is returned.
...
def create_collection(collection_data: CollectionCreate):
    ...
    This endpoint allows the creation of a new collection using the provided
    data.

    Args:
        collection_data (CollectionCreate): The data required to create a new
            collection, including name and description.
...
def delete_collection(collection_id: str):
    """Delete a collection and disassociate linked prompts.

    This endpoint deletes a specific collection using the provided ID. If the
    collection is found and successfully deleted, all associated prompts are
